...
// BRAINX_QUALITY_MACHINE_JSON_REJECT_20260702: extractor/reviewer LLM payloads
// ({"memories":[...]}, {"skill_candidates":[...]}) were stored VERBATIM as single
// memories (229 live rows, 195 hot avg imp 8.5 measured 2026-07-02) — BrainX eating
// its own exhaust; these outranked real memories in recall. A real memory is prose;
// a JSON envelope with an extraction key is machine output, never a memory.
function isMachineExtractionPayload(text) {
  const t = String(text || '').trim();
  if (!t.startsWith('{') && !t.startsWith('[')) return false;
  return /"(memories|skill_candidates|candidates|learning_details|extractions?)"\s*:\s*[\[{]/.test(t.slice(0, 300));
}
...
function assessMemoryQuality(memory = {}, overrides = {}) {
  ...
  if (isMachineExtractionPayload(original)) {
    return {
      action: 'skip',
      reason: 'machine_json_payload',
      score: -10,
      reasons: ['machine_json_payload'],
      tags: ['quality:rejected'],
      metrics: { chars: original.length, words: 0, signalHits: 0, uniqueRatio: 0 }
    };
  }
