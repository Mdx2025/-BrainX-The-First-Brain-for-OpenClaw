...
// (the OpenClaw runtime session copies). The Claude ACP fleet (arkham, versus, sonnet,
// blade, echo, raider, kyrus, xefora, clawma, cowboy, artemis, claude-cli, parker, ...)
...
// An ACP store whose <agent> is not a real fleet agent (e.g. ~/.claude-hello-mdx, the
...
function listAcpStores(agentFilter) {
  ...
  for (const name of entries) {
    ...
    if (!fleet.has(agent)) continue; // excludes hello-mdx, claude-cli-only-configs, etc.
