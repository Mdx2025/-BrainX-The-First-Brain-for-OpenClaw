...
Commands:
  ...
  add --type <type> --content <text> [--context <ctx>] [--tier <hot|warm|cold|archive>] [--importance <1-10>] [--tags a,b,c] [--agent <name>] [--id <id>]
      [--status <pending|in_progress|resolved|promoted|wont_fix>] [--category <learning|error|feature_request|correction|knowledge_gap|best_practice>]
...
async function cmdHealth(_args, deps = {}) {
  ...
  io.log(`BrainX V3 health: ${ok ? 'OK' : 'FAIL'}`);
