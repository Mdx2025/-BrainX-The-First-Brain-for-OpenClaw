...
// ── Reactive error-recall (BRAINX_REACTIVE_ERROR_RECALL_FINGERPRINT_20260719) ──
async function cmdErrorRecall(args, deps = {}) {
  const io = getIo(deps);
  const sub = args._?.[0] || args.sub;
  const jsonOutput = !!args.json;
  const er = require('./error-recall');

  if (sub === 'fingerprint') {
    const fp = er.computeFingerprint({ tool: args.tool, errorMessage: args.error || args.message, exitCode: args.exit ?? null });
    io.log(jsonOutput ? JSON.stringify({ fingerprint: fp }) : (fp || '(no error signal)'));
    return;
  }
  if (sub === 'backfill') {
    const res = await er.backfillFingerprints({ limit: args.limit ? parseInt(args.limit, 10) : 2000 });
    io.log(jsonOutput ? JSON.stringify(res) : `backfill: scanned=${res.scanned} updated=${res.updated}`);
    return;
  }
  if (sub === 'stats') {
    const res = await er.surfaceStats({ days: args.days ? parseInt(args.days, 10) : 7 });
    io.log(jsonOutput ? JSON.stringify(res, null, 2) : `error_recall ${res.days}d: lookups=${res.lookups} exact=${res.exact_hits} vector=${res.vector_hits} feedback=${res.with_feedback} resolved=${res.resolved} ratio=${res.resolved_ratio ?? 'n/a'}`);
    return;
  }
  if (sub === 'outcome') {
    if (!args.ids) throw new Error('--ids <comma-separated memory ids> is required');
    const memoryIds = String(args.ids).split(',').map((s) => s.trim()).filter(Boolean);
    const resolved = String(args.resolved) === 'true' || args.resolved === 'yes';
    const res = await er.recordOutcome({ memoryIds, resolved, lookupId: args.lookup || null });
    io.log(jsonOutput ? JSON.stringify(res) : `outcome recorded: updated=${res.updated} resolved=${resolved}`);
    return;
  }
  // default: lookup
  if (!args.tool && !(args.error || args.message)) {
    throw new Error('Usage: brainx error-recall <lookup|backfill|stats|fingerprint|outcome>\n  lookup --tool <t> --error "<msg>" [--exit <code>] [--agent <a>] [--project <p>] [--json]\n  backfill [--limit N]\n  stats [--days N] [--json]\n  fingerprint --tool <t> --error "<msg>" [--exit <code>]\n  outcome --ids <id1,id2> --resolved <true|false> [--lookup <lookupId>]');
  }
  const result = await er.lookup({
    tool: args.tool,
    errorMessage: args.error || args.message,
    exitCode: args.exit ?? null,
    agent: args.agent || process.env.OPENCLAW_AGENT || null,
    project: args.project || null,
    limit: args.limit ? parseInt(args.limit, 10) : 2,
  });
  if (jsonOutput) {
    io.log(JSON.stringify(result, null, 2));
  } else if (!result.text) {
    io.log(er.isEnabled() ? `No prior fix found (fingerprint: ${result.fingerprint || 'n/a'}).` : 'Reactive error-recall is disabled (set BRAINX_REACTIVE_ERROR_RECALL=1).');
  } else {
    io.log(`${result.text}\n\n(match: ${result.match_kind}, fingerprint: ${result.fingerprint}, id: ${result.id})`);
  }
}
...
async function main(argvIn = process.argv.slice(2), deps = {}) {
  ...
  // Reactive error-recall (fingerprint exact-match + vector fallback)
  if (cmd === 'error-recall') return cmdErrorRecall(args, deps);
