...
// BRAINX_STATUS_SNAPSHOT_TTL_SINGLEFLIGHT_READONLY_20260730: readOnly mode is
// used by the gateway brainx.status snapshot path. It skips the prune pass and
// the sanitize write-back so a status read never mutates memory; counts are
// computed from raw on-disk state (sanitize-affected states may count as open).
// Pruning still runs from withSessionState/buildPromptBlock on every turn.
async function getStats(options = {}) {
  const readOnly = options.readOnly === true;
  if (!readOnly) await maybePruneWorkingMemory();
  try {
