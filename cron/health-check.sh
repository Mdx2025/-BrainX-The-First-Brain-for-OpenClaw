#!/bin/bash
# BrainX V3 Health Check Cron Job
# Runs every 30 minutes to verify BrainX V3 status

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BRAINX_DIR="$(dirname "$SCRIPT_DIR")"
LOG_FILE="/home/clawd/.openclaw/skills/brainx-v3/cron/health.log"
...
if [ ! -f "$BRAINX_DIR/brainx-v3" ]; then
    log "ERROR: brainx-v3 CLI not found at $BRAINX_DIR/brainx-v3"
    exit 1
fi
...
# Run health check
log "Starting BrainX V3 health check..."

if "$BRAINX_DIR/brainx-v3" health > /tmp/brainx-health-output.txt 2>&1; then
    STATUS="OK"
